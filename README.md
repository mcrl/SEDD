...
Before using SEDD, ensure the following are installed on your system:
...
- Configure GPU Architecture Before running the build commands, open CMakeLists.txt and set the CMAKE_CUDA_ARCHITECTURES value to match your GPU.

Follow these steps to build SEDD:
```bash
mkdir build
cd build
cmake ..
...
**Note**: Our experiment was tested up to 8 nodes. In a multi-node environment, the performance was optimal when the total number of processes was kept under 32. For example, with 8 nodes, we used 4 processes per node.
